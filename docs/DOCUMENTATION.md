...
3.  [Architecture](#3-architecture)
    ...
    *   [Database](#database)
    *   [LLM Enhancements](#llm-enhancements)
    *   [Utilities](#utilities)
...
*   **Natural Language Understanding (NLU)**: Extracts key entities like location, hospital type, insurance provider, the number of hospitals to search for (`n_hospitals`), and the search radius (`distance_km`) from text. It leverages advanced intent handling. A fine-tuned QLoRA based LLM, in conjunction with the RAG system, provides precise insurance query processing, broader intent recognition, and assists in generating relevant, context-aware responses. This supports both spaCy (default) and LLM-based approaches for NLU.
...
*   **`recognize_query_tool`**:
    *   **Purpose**: Extracts structured data (intent, location, hospital type, insurance, number of hospitals, search radius) from a given query text, with enhanced intent handling and insurance query processing capabilities.
    *   **Implementation**: Utilizes `tools/recognize.py`, integrating both spaCy and the fine-tuned QLoRA based LLM for robust entity and intent recognition.
*   **`rag_retrieve_tool`**:
    *   **Purpose**: Fetches contextually relevant information from the Vector Database (Vectordb) to augment LLM responses, particularly for complex queries.
    *   **Implementation**: Utilizes `tools/rag_retrieve.py` for RAG capabilities.
*   **`text_to_speech_tool`**:
...
*   **SQLite Database**: (`data/hospitals.sqlite`) managed by `Pony ORM`.
...
*   **Vector Database (Vectordb)**: (`data/vdb_hospitals/`) used for Retriever-Augmented Generation (RAG).
    *   **Purpose**: Stores vectorized representations of hospital and insurance-related information, enabling semantic search and contextual retrieval for the fine-tuned LLM.
    *   **Initialization**: The Vectordb is generated and populated by `db/modules/vector_db_generator.py`.

### LLM Enhancements
*   **Fine-tuned QLoRA based LLM**: Integrated primarily for enhanced response generation and intent handling.
    *   **Purpose**: Specialized in handling insurance queries and general intent recognition, working in tandem with the RAG system to produce highly accurate, relevant, and contextually appropriate responses.
    *   **Training**: Trained using QLoRA techniques and custom datasets for insurance-related dialogues, managed by `db/modules/fine_tune.py` and `db/modules/fine_tune_data_generator.py`.
...
*   spaCy English model:
    ...
    ```
*   **Vector Database (Vectordb) Initialization**:
    *   To leverage the RAG capabilities, ensure the Vectordb is populated. This can typically be done by running `db/modules/vector_db_generator.py`.
...
*   **Vectordb issues**: If RAG-based responses are not as expected, ensure `db/modules/vector_db_generator.py` has been run and the embeddings are correctly generated.
...
*   `sentence-transformers`, `faiss-cpu`: For Vectordb and RAG functionalities.
*   `loralib`, `accelerate`, `bitsandbytes`, `transformers`: For QLoRA and fine-tuning.
